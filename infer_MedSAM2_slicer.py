...
def infer_3d(predictor, img_npz_file, gts_file, propagate, model_cfg, pred_save_dir):
    ...
    if np.max(img_3D) >= 256:
        print("⚠ 입력값이 0~255 범위 이상 → 정규화 수행")
        img_3D = (img_3D - np.min(img_3D)) / (np.max(img_3D) - np.min(img_3D)) * 255
        img_3D = img_3D.astype(np.int16)
    print(f"정규화 후 min={img_3D.min()}, max={img_3D.max()}")

    # --- RGB 변환 ---
    img_3D = grayscale2rgb(img_3D)  # (D, H, W, 3)
    D, H, W = img_3D.shape[:3]
    print(f"RGB 변환 후 img_3D.shape={img_3D.shape} (D={D}, H={H}, W={W})")
    ...
    print(f"모델 image_size={image_size}")

    # --- 이미지 리사이즈 ---
    img_resized = resize_rgb(img_3D, image_size) / 255.0
    img_resized = torch.from_numpy(img_resized).cuda()
    print(f"img_resized.shape={img_resized.shape}, dtype={img_resized.dtype}")
    ...
    # --- z_range 확인 ---
    z_min, z_max, slice_idx = z_range
    print(f"z_min={z_min}, z_max={z_max}, slice_idx={slice_idx}, 전체 깊이 D={D}")

    labels = np.unique(gts) if gts is not None else [1]
    print(f"라벨 목록: {labels}")
    ...
    for idx, label in enumerate(labels, start=1):
        ...
        if indices[0].size == 0:
            print(f"라벨 {label}: GT 없음, 건너뜀")
            continue

        print(f"indices[0] (z축 좌표) min={indices[0].min()}, max={indices[0].max()}, count={len(indices[0])}")
        ...
        print(f"잘라낸 img.shape={img.shape}, 깊이={img.shape[0]}")

        ann_frame_idx = z_mid_orig - z_min
        print(f"ann_frame_idx={ann_frame_idx}, 참조가능범위=[0, {img.shape[0]-1}]")

        if not (0 <= ann_frame_idx < img.shape[0]):
            print(f"❌ ann_frame_idx {ann_frame_idx} 가 범위를 벗어남!")
            ann_frame_idx = max(0, min(ann_frame_idx, img.shape[0]-1))

        mask_prompt = gt[z_mid_orig]
        print(f"mask_prompt.shape={mask_prompt.shape}, unique={np.unique(mask_prompt)}")

        video_height, video_width = H, W

        with torch.inference_mode(), torch.autocast("cuda", dtype=torch.bfloat16):
            inference_state = predictor.init_state(img, video_height, video_width)
            print("✅ predictor.init_state 완료")

            frame_idx, object_ids, masks = predictor.add_new_mask(
                inference_state, frame_idx=ann_frame_idx, obj_id=1, mask=mask_prompt
            )
            print(f"✅ add_new_mask 성공 frame_idx={frame_idx}, object_ids={object_ids}")
            ...
            for out_frame_idx, out_obj_ids, out_mask_logits in predictor.propagate_in_video(inference_state):
                print(f"  ▶ forward out_frame_idx={out_frame_idx}, 범위=[{z_min},{z_max}]")
            ...
            for out_frame_idx, out_obj_ids, out_mask_logits in predictor.propagate_in_video(inference_state, reverse=True):
                print(f"  ▶ backward out_frame_idx={out_frame_idx}, 범위=[{z_min},{z_max}]")
                segs_3D[(z_min + out_frame_idx), (out_mask_logits[0] > 0.0).cpu().numpy()[0]] = label

            predictor.reset_state(inference_state)

    print("세그멘테이션 완료, 결과 unique:", np.unique(segs_3D))
    ...
    #             print(out_frame_idx)
    #             segs_3D[(z_min + out_frame_idx), (out_mask_logits[0] > 0.0).cpu().numpy()[0]] = label
    #             # segs_3D[(out_frame_idx), (out_mask_logits[0] > 0.0).cpu().numpy()[0]] = label
    #         predictor.reset_state(inference_state)
    #         frame_idx, object_ids, masks = predictor.add_new_mask(inference_state, frame_idx=ann_frame_idx, obj_id=1, mask=mask_prompt)
    #         # 역방향전파
    #         for out_frame_idx, out_obj_ids, out_mask_logits in predictor.propagate_in_video(inference_state, reverse=True):
